...
function App() {
  ...
  const [format, setFormat] = useState<'string' | 'base64' | 'hex'>('string');
  ...
  const generate = () => {
    ...
    if (format === 'string') {
        ...
        // Simple base64 encoding of random bytes
        retVal = btoa(String.fromCharCode(...array.slice(0, Math.floor(length / 1.33))));
        setEntropy(calculateEntropy(length, 64));
    } else if (format === 'hex') {
        retVal = Array.from(array).map(b => b.toString(16).padStart(2, '0')).join('');
        setEntropy(calculateEntropy(length, 16));
    }
  ...
  useEffect(() => {
    generate();
  }, [length, useUpper, useLower, useNumbers, useSymbols, format]);

  return (
    <div style={{ padding: '2rem', maxWidth: '600px', margin: '0 auto', fontFamily: 'sans-serif' }}>
      <h1>Random String & Secret Generator</h1>
      ...
      <div style={{ marginBottom: '1rem' }}>
        ...
        <select value={format} onChange={(e) => setFormat(e.target.value as any)} style={{ width: '100%', padding: '0.5rem' }}>
            <option value="string">String</option>
            <option value="base64">Base64</option>
            <option value="hex">Hex</option>
        </select>
      </div>
      <div style={{ margin: '1rem 0', display: format === 'string' ? 'block' : 'none' }}>
